      ...
      <div class="form-group col-md-6 col-12">
       ...
       <div class="form-group col-md-12 col-12">
         ...
         <select class="form-control" name="Emp_ID"  >
            ...
            <option value="{{ $employee->ID }}" {{ $employee->ID == $qury->Emp_ID ? 'selected' : '' }}>{{ $employee->Emp_FirstName }} {{ $employee->Emp_LastName }}</option>
            @endforeach
         </select>
      </div>
      <div class="form-group col-md-6 col-12">
                      ...
                      <table class="table table-striped " id="sortable-table">
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td><input type="hidden" value="{{ $timeTabl->ID }}" name="id[]" class="form-control"></td>
                            <td><label>Day</label>
                                 <select class="form-control" name="Day[]"  >
                                    @foreach($days as $day)
                                    <option value="{{ $day }}" {{ $day == $timeTabl->Day ? 'selected' : '' }}>{{ $day}}</option>
                                    @endforeach
                                 </select></td>
                            <td> <label>Start Time</label>
                                 <input type="time" value="{{ $timeTabl->StartTime }}" name="StartTime[]" class="form-control"></td>
                            <td> <label>End Time</label>
                                 <input type="time" value="{{ $timeTabl->EndTime }}" name="EndTime[]" class="form-control"></td>
                            <td><label>Building</label>
                                 <select class="form-control" name="Building[]"  required>
                                    @foreach ($building as $l) 
                                    <option value="{{ $l }}" {{ $l == $timeTabl->Building ? 'selected' : '' }}>{{ $l }}</option>
                                    @endforeach
                                 </select>
                           </td>
                              <td><label>Room</label>
                                  <select class="form-control" name="room[]"  required>
                                    ...
                                    <option value="{{ $l }}" {{ $l == $timeTabl->Room ? 'selected' : '' }}>{{ $l }}</option>
                                    @endforeach
                                 </select></td>
                            <!--   <td><label>Copy Row</label>
                                <a style="color: white;" class="btn btn-warning btn-block clone">{{ 'Add New Row' }}</a></td> -->
