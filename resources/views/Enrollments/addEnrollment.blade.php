                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-right">
                            <ul>
                                ...
                                <li class="pb-8">
                                    <p class="badge badge-success badge-shadow">Allowed Credit Hours:
                                        {{ $acdRule['creditHoursAllowed'] }}</p>
                                </li>
                                @if ($getTotalCreditHours > 0)
                                    <li>
                                        <p class="badge badge-success badge-shadow"> Remaining Credit Hours:
                                            {{ $acdRule['creditHoursAllowed'] - $getTotalCreditHours }}</p>
                                    </li>
                                @else
                                    <li>
                                        <p class="badge badge-danger badge-shadow">Remaining Credit Hours:
                                            {{ $acdRule['creditHoursAllowed'] - $getTotalCreditHours }}</p>
                                    </li>
                                @endif
                            </ul>
                                                            ...
                                                            @foreach ($DegsemesterCourses as $DegsemesterCourse)
                                                                @if (in_array($DegsemesterCourse->semesterCourse->ID, $enrollmentsArray))
                                                                    <tr>
                                                                        ...
                                                                        <td>{{ $DegsemesterCourse->semesterCourse->course->CourseCode ?? '--' }}
                                                                            <!-- <div class="badge badge-success">Active</div> -->
                                                                        </td>
                                                                        <td>{{ $DegsemesterCourse->semesterCourse->course->CourseName ?? '--' }}
                                                                        </td>
                                                                        <td>{{ $DegsemesterCourse->semesterCourse->course->CreditHours ?? '--' }}
                                                                        </td>
                                                                        <td>{{ $DegsemesterCourse->semesterCourse->degreeBatches->degree->DegreeName ?? '--' }}
                                                                        </td>
                                                                        <td>{{ $DegsemesterCourse->semesterCourse->semester->SemSession ?? '--' }}
                                                                        </td>
                                                                        <td>{{ $DegsemesterCourse->Section ?? '--' }}
                                                                        </td>
                                                                    </tr>
                                                                @endif
                                                            @endforeach
                                            ...
                                            <div class="badge badge-success">Active</div>
                                                                    ...
                                                                    <td>
                                                                        @if ($enrollments->sum('Status') == 0)
                                                                            <a href="{{ route('store.Enrollment', $enrollment->SemCourses_ID . '-true') }}"
                                                                                class="btn btn-sm btn-danger">Drop
                                                                                Course</a>
                                                                        @else
                                                                    ...
                                                                    <td>{{ $enrollment->semesterCourse->course->CourseCode ?? '--' }}
                                                                    </td>
                                                                    <td>{{ $enrollment->semesterCourse->course->CourseName ?? '--' }}
                                                                    </td>
                            ...
                            <div class="card sbmt-form">
                                <div class="card-header justify-content-center alert alert-danger">
                                    <a data-toggle="modal" data-target="#exampleModal" style="color: white;"
                                        class="btn btn-primary">Confirm Enrollment</a>
                                </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        After Clicking the Confirm Button you can not Add Or Drop Courses So Please Be Carefully Click The
                        Confim Button
                    </div>
